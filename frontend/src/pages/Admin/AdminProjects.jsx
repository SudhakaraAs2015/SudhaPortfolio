  ...
  const onFinish = async (values) => {
    try {
      ...
      if (selectedItemForEdit) {
        response = await axios.put("/api/sudhaportfolio/update-projects", {
          ...values,
          _id: selectedItemForEdit._id,
        });
      } else {
        response = await axios.post("/api/sudhaportfolio/add-projects", values);
      }
  ...
  const onDelete = async (item) => {
    try {
      ...
      const response = await axios.delete(
        "/api/sudhaportfolio/delete-projects",
        {
          data: { _id: item._id },
        }
      );
          ...
          >
            ...
            <div className="flex justify-end gap-4 mt-5">
              ...
              <button
                ...
                onClick={() => {
                  setSelectedItemForEdit(project); // Set the selected item
                  setShowAddEditModel(true); // Show the modal
                  form.setFieldsValue({
                    ...project,
      ...
      {showAddEditModel && ( // Only show the modal when it is true
        ...
        >
          <Form
            ...
            initialValues={{
              ...selectedItemForEdit,
              technologies: selectedItemForEdit?.technologies?.join(", ") || "", // Ensure it initializes correctly
            }}
          >
            <Form.Item
              name="title"
